    ...
    while verzik.is_phase_active():
        ...
        if verzik.purple_crab_active:
            ...
            if mager.attack_cooldown == 0:
                if verzik_attacking:
                    # Switch to Tumeken’s Shadow loadout
                    switch_player_to_loadout(mager, loadouts["mage_8_way"])
...
def run_multiple_simulations_series(num_iterations=100, output_csv="p2_results.csv"):
    """
    Run the single-phase simulation multiple times
    and record the results to a CSV.
    """
    # Open CSV file
    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
